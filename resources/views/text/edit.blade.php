...
@section('title', 'Edit Bulk SMS Campaign')
...
<div class="card">
    ...
    <div class="card-body">
        <form action="{{ route('text.update',$text->id) }}" method="POST" enctype="multipart/form-data" id="smsForm">
            @csrf
            @method('PUT')
            <div class="form-group">
                <label for="title">Title of SMS / Campaign*</label>
                <input type="text" name="title" id="title" class="form-control" value="{{ $text->text_title }}"
            ...
            <div class="form-group">
                ...
                <div class="d-flex align-items-center flex-wrap">
                    <div class="form-check mr-3">
                        <input class="form-check-input" type="radio" name="contact_source_edit" value="manual" id="manual"
                            {{ old('contact_source', $text->contact_type) == 'manual' ? 'checked' : 'readonly' }}>
                        <label class="form-check-label" for="manual">Enter Recipient Contacts</label>
                    </div>
                    <div class="form-check mr-3">
                        <input class="form-check-input" type="radio" name="contact_source_edit" value="csv" id="csv"
                            {{ old('contact_source', $text->contact_type) == 'csv' ? 'checked' : 'readonly' }}>
                        <label class="form-check-label" for="csv">Import from a CSV File</label>
                    </div>
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="contact_source_edit" value="saved" id="saved"
                            {{ old('contact_source', $text->contact_type) == 'saved' ? 'checked' : 'readonly' }}>
            ...
            <div class="form-group" id="manual-input-edit"
            @if($text->contact_type == 'manual') style="display:block" @else style="display:none" @endif>
            <label for="contacts">Enter Contacts (comma-separated)</label>
            <textarea name="contacts" id="contacts" class="form-control" rows="2"
                placeholder="Enter the contacts comma separated e.g. 0712345678,0714345678">{{old('contacts', $text->recepient_contacts) }}</textarea>
        </div>

        <div class="form-group" id="csv-upload-edit"
            @if($text->contact_type == 'csv') style="display:block" @else style="display:none" @endif>
            <button type="button" class="btn btn-secondary btn-sm" data-toggle="modal" data-target="#csvModal">
                UPLOAD CSV FILE
            </button>
            <div class="csv_file_details mt-2"></div>

            <input type="hidden" name="csv_file_name" id="csv_file_name" value="{{$text->csv_file_name}}">
            <input type="hidden" name="csv_file_path" id="csv_file_path" value="{{$text->csv_file_path}}">
            <input type="hidden" name="csv_file_columns" id="csv_file_columns" value="{{$text->csv_file_columns}}">
        </div>

        @php
            $selectedLists = json_decode($text->contact_list ?? '[]', true);
        @endphp

        <div class="form-group" id="saved-list"
            @if($text->contact_type == 'saved') style="display:block" @else style="display:none" @endif>
            <label for="contact_list">Select Contact List</label>
            <select name="contact_list[]" id="contact_list" class="select2 form-control" multiple style="width:100%">
                <option value="">Select a list</option>
                @foreach($contactLists as $key => $list)
                    <option value="{{ $key }}" {{ in_array($key, $selectedLists) ? 'selected' : '' }}>
                        {{ $list }}
                    </option>
                @endforeach
            </select>
            ...
            <div class="form-group">
                ...
                <textarea name="message" id="message" class="form-control" rows="4"
                    placeholder="Enter your text message">{{ old('message', $text->message) }}</textarea>
            </div>
            ...
            <div class="form-group">
                <div class="form-check">
                    <input class="form-check-input" type="checkbox" id="schedule" name="schedule"
                        {{ old('schedule', $text->scheduled) ? 'checked' : '' }}>
                    <label class="form-check-label" for="schedule">Schedule SMS</label>
                </div>
            </div>

            <div class="form-group {{ $text->scheduled ? '' : 'd-none' }}" id="schedule-fields">
                ...
                <input type="text" name="schedule_date" id="schedule_date" class="form-control-sm datepicker"
                    ...
                    value="{{ old('schedule_date', $text->schedule_date ? \Carbon\Carbon::parse($text->schedule_date)->format('d/m/Y') : '') }}">
                ...
                <input type="text" name="schedule_time" id="schedule_time" class="form-control-sm timepicker"
                    ...
                    value="{{ old('schedule_time', $text->schedule_date ? \Carbon\Carbon::parse($text->schedule_date)->format('h:i A') : '') }}">
            </div>
...
<script>
    ...
    $('input[name="contact_source_edit"]').on('change', function() {
        $('#manual-input-edit, #csv-upload-edit, #saved-list').addClass('d-none');
        if ($(this).val() === 'manual') $('#manual-input-edit').removeClass('d-none');
        if ($(this).val() === 'csv') $('#csv-upload-edit').removeClass('d-none');
...
$('#smsForm').on('submit', function(e) {
    ...
    const recipientMethod = $('input[name="contact_source_edit"]:checked').val();
    ...
    recipientMethod: $('input[name="contact_source_edit"]:checked').val(),
    contacts: $('#contacts').val().trim(),
    csvFilePath: $('#csv_file_path').val().trim(),
    contactList: $('#contact_list').val(),
    message: $('#message').val().trim(),
    isScheduled: $('#schedule').is(':checked'),
    scheduleDate: $('#schedule_date').val().trim(),
    scheduleTime: $('#schedule_time').val().trim()
});
...
document.addEventListener('DOMContentLoaded', function () {
    let scheduleCheckbox = document.getElementById('schedule');
    let scheduleFields = document.getElementById('schedule-fields');
...
document.addEventListener("DOMContentLoaded", function () {
    document.querySelectorAll('input[name="contact_source_edit"]').forEach(input => {
        if (!input.checked) {
            input.disabled = true;
        }
    });
...
$(document).ready(function() {
    let csvFileName = $('#csv_file_name').val();
    let csvFilePath = $('#csv_file_path').val();
    let csvFileColumns = $('#csv_file_columns').val();

    if (csvFileName || csvFilePath || csvFileColumns) {
        $('.csv_file_details').html(`
            <p><strong>File Name:</strong> ${csvFileName}</p>
            <p><strong>File Path:</strong> <a href="/${csvFilePath}" download="${csvFileName}" class="btn btn-sm">Download</a></p>
