...
class PTPReminderJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        try {
            // Get PTP activities due today (activity_type_id = 4) for leads with institution client_contract_type_id = 1
            $ptpActivities = Activity::where('activity_type_id', 4)
                ...
                ->whereHas('lead', function($query) {
                    $query->whereHas('institution', function($institutionQuery) {
                        $institutionQuery->where('client_contract_type_id', 1);
                    });
                })
                ->with(['lead'])
                ->get();
            ...
            Log::info("Found {$totalCustomers} PTP customers for date: {$this->processDate} (filtered by act_ptp_date and client_contract_type_id = 1)");
